...
async function bootstrap() {
  ...
  app.enableCors();
  app.getHttpAdapter().get('/health', (req, res) => res.json({ status: 'ok', service: 'messaging-service' }));
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: { package: 'messaging', protoPath: process.env.PROTO_PATH || '/proto/messaging.proto', url: `0.0.0.0:${process.env.GRPC_PORT || 50054}` },
  });
  await app.startAllMicroservices();
  const port = Number(process.env.PORT ?? 3005);
  await app.listen(port);
  console.log(`💬 Messaging Service HTTP :${port} gRPC :${process.env.GRPC_PORT || 50054}`);
  await registerConsul('messaging-service', port);
}
bootstrap();