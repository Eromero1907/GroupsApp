...
async function bootstrap() {
  ...
  app.enableCors();
  app.getHttpAdapter().get('/health', (req, res) => res.json({ status: 'ok', service: 'users-service' }));
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: { package: 'users', protoPath: process.env.PROTO_PATH || '/proto/users.proto', url: `0.0.0.0:${process.env.GRPC_PORT || 50052}` },
  });
  await app.startAllMicroservices();
  const port = Number(process.env.PORT ?? 3002);
  await app.listen(port);
  console.log(`👤 Users Service HTTP :${port} gRPC :${process.env.GRPC_PORT || 50052}`);
  await registerConsul('users-service', port);
}
bootstrap();