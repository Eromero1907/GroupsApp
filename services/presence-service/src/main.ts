...
async function bootstrap() {
  ...
  app.enableCors();
  app.getHttpAdapter().get('/health', (req, res) => res.json({ status: 'ok', service: 'presence-service' }));
  ...
  console.log(`🟢 Presence Service corriendo en http://localhost:${port}`);
  await registerConsul('presence-service', port);
}
bootstrap();